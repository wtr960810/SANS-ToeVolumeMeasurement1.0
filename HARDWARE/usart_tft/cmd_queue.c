...
#define U3_CMD_HEAD 0XAABB                                            //帧头
#define U3_CMD_TAIL 0XCCDD                                            //帧尾
...
static QUEUE1  U3_que = {0,0,0};                                            //指令队列
static uint16_t U3_cmd_heard = 0;                                           //队列帧尾检测状态
static uint16_t U3_cmd_state = 0;                                           //队列帧尾检测状态
static qsize  U3_cmd_pos = 0;                                              //当前指令指针位置
/*! 
*  \brief  清空指令数据
*/
void queue_reset()
...
{
    if(U3_que._tail!=U3_que._head)                                     //非空状态
    {
...
qsize U3_queue_find_cmd(qdata *buffer,qsize buf_len)
{
    ...
    while(U3_queue_size()>0)
    {
        //取一个数据
        U3_queue_pop(&_data);
			  if(U3_cmd_pos==0)                                                                //指令第一个字节必须是帧头，否则跳过
        {
			   U3_cmd_heard=_data;
				}
        if(U3_cmd_pos==1&&((U3_cmd_heard<<8)|_data)!=U3_CMD_HEAD)                        //指令第一个字节必须是帧头，否则跳过
        {
					  U3_cmd_pos=0;
            continue;
        }
        if(U3_cmd_pos<buf_len)                                           //防止缓冲区溢出
            buffer[U3_cmd_pos++] = _data;
        U3_cmd_state = ((U3_cmd_state<<8)|_data);                           //拼接最后2个字节，组成一个16位整数
        //最后4个字节与帧尾匹配，得到完整帧
        if(U3_cmd_state==U3_CMD_TAIL)
        {
					  
            cmd_size = U3_cmd_pos;                                       //指令字节长度
            U3_cmd_state = 0;                                            //重新检测帧尾巴
            U3_cmd_pos = 0;                                              //复位指令指针

#if(CRC16_ENABLE)
            //去掉指令头尾AABB，尾CCDD共计4个字节，只计算数据部分CRC
            if(!CheckCRC16(buffer+1,cmd_size-4))                      //CRC校验
                return 0;
