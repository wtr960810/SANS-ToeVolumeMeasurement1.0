...
//重定义fputc函数 
int fputc(int ch, FILE *f)//使Printf 映射到USART端口输出
{      
...
#if (EN_USART1_RX|EN_USART2_RX|EN_USART3_RX|EN_USART4_RX)   //如果使能了接收
//串口1中断服务程序
...
//    for(i=0;i<size;i++)
//	{
//	   USART_SendData(UART4, string[i]);//向串口2发送数据
...
void USART2_SENDString(u8 *string,u16 size)
 {	u8 i;
    ...
    for(i=0;i<size;i++)
	{
	   USART_SendData(USART2, string[i]);//向串口2发送数据
...
//    for(i=0;i<size;i++)
//	{
//	   USART_SendData(USART3, string[i]);//向串口2发送数据
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
    ...
    if(USART1_RX_STA&0x8000)
			{if((USART1_RX_BUF[0]==0x4d)&&(USART1_RX_BUF[1]==0x4f))//判断握手
				{ 
				  USART1_SENDString(Hand,8);//发送握手信号							
				}
				if((USART1_RX_BUF[0]==0x47)&&(USART1_RX_BUF[1]==0x44))//判断抓取数据
				{
				    if(StartFlage==0)
							{ delay_ms(5);
								COM=1;
							...
							{
							 COM=0;
				       USART1_SENDString(GetData,8);//向串口2发送数据						
							}
				}
				USART1_RX_STA=0;
			}
